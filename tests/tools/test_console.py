...
async def test_telnet_node_single_string_command_is_wrapped_in_a_list() -> None:
    ...
    client.list_lab_nodes.return_value = {
        "status": "success",
        "data": {"status": 2, "console": "telnet", "url": "telnet://192.168.1.50:41041"},
    }
...
async def test_telnet_node_requires_node_to_be_running() -> None:
    ...
    client.list_lab_nodes.return_value = {
        "status": "success",
        "data": {"status": 0, "console": "telnet", "url": "telnet://192.168.1.50:41041"},
    }
...
async def test_telnet_node_requires_telnet_console_type() -> None:
    ...
    client.list_lab_nodes.return_value = {
        "status": "success",
        "data": {"status": 2, "console": "vnc", "url": "vnc://192.168.1.50:5900"},
    }
...
async def test_telnet_node_success_calls_telnet_session_with_parsed_host_port() -> None:
    ...
    mock_session.assert_awaited_once_with("192.168.1.50", 41041, ["vlan 20", "name testing"], idle_timeout=3.0)
    assert result["status"] == "success"
...
async def test_telnet_node_connection_failure_is_reported_as_error() -> None:
    ...
    async def _raise(*args, **kwargs):
        raise ConnectionError("Could not connect to 192.168.1.50:41041 after 3 attempt(s)")
